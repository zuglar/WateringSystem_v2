...
class CustomDialog {
    ...
    render = function (type, title, message, color, callback) {
        ...
        if (type === 0) {
            // one button dialog - Alert
            document.getElementById('dialogboxfoot').innerHTML = '<button id="yes" style="background:' + color + '">Ok</button>';
        } else if (type === 1) {
            // two buttons Yes / No dialog - Confirm
            ...
            // three buttons Save / Delete / Cancel dialog - Confirm
...
export { warningColor, errorColor, okColor, scalePage, getvals, CustomDialog };
